...
const faqs = [
  ...
  },
  {
    ...
    answer: "Yes! We offer a lifetime warranty on all our masonry work. We stand behind our craftsmanship and use only premium materials to ensure your project lasts generations."
  },
  {
    ...
    <section className="py-24 bg-stone-gradient">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <p className="text-copper font-medium tracking-[0.3em] uppercase text-sm mb-4">Common Questions</p>
          <h2 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">Frequently Asked Questions</h2>
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className="border border-copper/20 rounded-lg overflow-hidden bg-card hover:border-copper/40 transition-all duration-300"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full px-6 py-5 flex items-center justify-between hover:bg-copper/5 transition-colors"
              >
                <h3 className="font-display text-lg font-semibold text-foreground text-left">{faq.question}</h3>
                <ChevronDown
                  className={`w-5 h-5 text-copper flex-shrink-0 ml-4 transition-transform duration-300 ${
              ...
              {openIndex === index && (
                <div className="px-6 py-4 bg-copper/5 border-t border-copper/20">
