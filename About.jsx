    ...
    <section id="about" className="py-24 bg-charcoal-gradient">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="grid grid-cols-2 gap-8">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center p-6">
                <p className="font-display text-4xl md:text-5xl font-bold text-copper mb-2">{stat.number}</p>
                <p className="text-cream/60 text-sm tracking-wide uppercase">{stat.label}</p>
              </div>
            ))}
          </div>
          ...
          {/* Text */}
          <div>
            <p className="text-copper font-medium tracking-[0.3em] uppercase text-sm mb-4">Our Story</p>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-cream mb-6">
              Craftsmanship Rooted in Tradition
            </h2>
            <p className="text-cream/70 leading-relaxed mb-6">
              For over two decades, Akmens Burtininkas has been the trusted name in natural stone masonry. 
